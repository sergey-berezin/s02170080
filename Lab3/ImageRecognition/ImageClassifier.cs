...
namespace ImageRecognition
{
    public class ImageClassifier
    {
        ...
        private static readonly string[] classLabels =
                File.ReadAllLines("..\\..\\..\\classLabels.txt");
                    ...
                    if (query.Count() > 0)
                    {
                        foreach (var a in query)
                            lock (lockObj)
                            {
                                ...
                                predictionOutputs.Enqueue(new ImageResult(path, a.OutputLabel, a.Confidence, new ImageDetails(blob)));
                            }
                    }
                    else
                    {
                        ...
                        lock (lockObj)
                        {
                            ...
                            {
                                byte[] blob = File.ReadAllBytes(path);
                                predictionOutputs.Enqueue(new ImageResult(path, p.Label, p.Confidence, new ImageDetails(blob)));
